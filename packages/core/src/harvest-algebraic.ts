...
import { ΛWave, Φ, HarvestResult, TopologicalTransformer, PhaseState } from './types.js';
import {
  lifecycleAlgebra,
  deconstructionPhase,
  synthesisPhase,
} from './dipoles-algebraic.js';
 ...
 */
function crystallize(wave: ΛWave, field: Φ): TopologicalTransformer | null {
 ...
 */
function updateField(field: Φ, transformer: TopologicalTransformer | null): Φ {
  if (!transformer) {
 ...
 */
export function µ_HARVEST_pure<T = any, R = any>(
  seed: ΛWave<T, R>,
  field: Φ
): ΛWave<T, R> {
 ...
 */
export function µ_HARVEST_phases<T = any, R = any>(
  seed: ΛWave<T, R>,
  field: Φ
): {
 ...
 */
export function validateHarvestAlgebra(field: Φ): {
  isMonoid: boolean;
  ...
  const isMonoid =
    lifecycleAlgebra.properties.associative &&
