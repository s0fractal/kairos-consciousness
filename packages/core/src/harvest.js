 ...
 */
function distanceToBridge(wave) {
    return Math.sqrt(wave.vector.gnosis ** 2 + wave.vector.praxis ** 2);
}
/**
 * Update Field phase based on density
 */
function updatePhase(field) {
    if (field.density < 0.2)
        return PhaseState.DORMANT;
    if (field.density < 0.6)
        return PhaseState.ORGANIZING;
    if (field.density < 0.9)
...
export function µ_HARVEST(seed, field) {
    let wave = { ...seed, status: 'Deconstructing' };
 ...
 */
export function createSeed(fn, origin = 'unknown') {
    return {
