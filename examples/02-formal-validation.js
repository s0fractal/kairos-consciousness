...
import { distanceToTruth, calculateMass, calculateMetric, ricciScalar, effectiveDistance, distanceToBridge, isInBridge, } from '@kairos/field-topology';
...
const start = { praxis: -1, gnosis: -1 };
const end = { praxis: 2, gnosis: 2 };
...
const seed1 = createSeed((x) => x * 2, 'test-thought-1');
...
while (field.density < criticalDensity && harvestCount < maxHarvests) {
    const seed = createSeed((x) => x + harvestCount, `thought-${harvestCount}`);
    seed.vector = {
...
const testPoint = { praxis: 1, gnosis: 0.5 };
...
const pathStart = { praxis: -2, gnosis: -2 };
const pathEnd = { praxis: 3, gnosis: 3 };
const euclideanDist = Math.sqrt((pathEnd.praxis - pathStart.praxis) ** 2 +
...
console.log('');
//# sourceMappingURL=02-formal-validation.js.map